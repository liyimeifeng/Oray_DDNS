...
void Java_com_li_ddns_Phddns_run(JNIEnv* env, jobject thiz, jstring jszHost, jstring jszUserID, jstring jszUserPWD, jstring jnicName, jstring jlogfile)
{
	const char *szHost = (*env)->GetStringUTFChars(env, jszHost, false);
	const char *szUserID = (*env)->GetStringUTFChars(env, jszUserID, false);
	const char *szUserPWD = (*env)->GetStringUTFChars(env, jszUserPWD, false);
	const char *nicName = (*env)->GetStringUTFChars(env, jnicName, false);
	const char *logfile = (*env)->GetStringUTFChars(env, jlogfile, false);
...
// //		printf ("Parameter has an unexpected error,please read the help above.Thank you for your patient!\n");
	...
	// }
	printf("start7");
//	initParameter2(&global,&parameter,szHost,szUserID,szUserPWD,nicName,logfile);
	printf("start8");
	if( parameter.bDaemon == 1 ) {
#ifndef WIN32
		printf("phddns started as daemon!\n");
...
#include "official/official_key.c"
	...
	(*env)->ReleaseStringUTFChars(env, jszHost, szHost);
	(*env)->ReleaseStringUTFChars(env, jszUserID, szUserID);
	(*env)->ReleaseStringUTFChars(env, jszUserPWD, szUserPWD);
	(*env)->ReleaseStringUTFChars(env, jnicName, nicName);
